...
public class CV2NewDataTest {



	  
	private static final Logger log = Logger.getLogger(CV2NewDataTest.class);
	static SimpleDateFormat sdf = new SimpleDateFormat("ddMMyyHHmmss");
	public static class HandlerCV2NewDataTest extends SimpleChannelUpstreamHandler  {
		...
		public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
			...
			String msg= (String)e.getMessage();
		 	...
		 	log.info("CV2NewDataTest:: Msg :Len: "+msg);
			String[] msgary = msg.split(",");
			DecimalFormat twoDForm = new DecimalFormat("#.##");
			String imei=msgary[0].substring(5);
			//log.info("CV2NewDataTest "+imei+" IMEI :: "+imei);
			String datestr=msgary[10]+msgary[2];
			//log.info("CV2NewDataTest "+imei+" DATE :: "+datestr);
			
			
			if (msgary[1].contains("$GPRMC")) {
				log.info("CV2NewDataTest "+imei+" :: "+msg);
			//	log.info("CV2NewDataTest "+imei+" GPS :: $GPRMC String");
				...
				System.out.println(imei);
				 ...
				 JSONObject parameters = new JSONObject(profile.getParameters());
			      ...
			      {  
			    	  log.info("CV2NewDataTest "+imei+" IN FORMULA  :: "+obj.get("Analogformula").toString());
			      ...
			      log.info("CV2NewDataTest "+imei+" GPS Main INPUTS :: "+digitalData);
			      log.info("CV2NewDataTest "+imei+" GPS GREEN ACMAINS_FAIL :: "+digitalData.substring(1,2));
			      log.info("CV2NewDataTest "+imei+" GPS Yellow Ornge Fire ::"+digitalData.substring(2,3));
			      log.info("CV2NewDataTest "+imei+" GPS Yellow Door Door :: "+digitalData.substring(3,4));
			      log.info("CV2NewDataTest "+imei+" GPS Yellow Black DG_Running_Hrs :: "+digitalData.substring(4,5));
			      log.info("CV2NewDataTest "+imei+" GPS Yellow Brown DG_Fault :: "+digitalData.substring(5,6));
			      log.info("CV2NewDataTest "+imei+" GPS Green Black Battry_Low :: "+digitalData.substring(6,7));
			      log.info("CV2NewDataTest "+imei+" GPS Yellow Red :: PP_Input_Fail "+digitalData.substring(7,8));
				...
				//	String battry=msgary[15];
					...
					double battry=Double.parseDouble(msgary[15].toString()) * analogFormula;
					 log.info("CV2NewDataTest "+imei+" IN FORMULA>>>>>> :: "+msgary[16]+" :: "+fuel);
					 log.info("CV2NewDataTest "+imei+" IN FORMULA>>>>>> :: "+battry);
					//DeviceProfile profile=device.getDp();
				Lasttrack track = lasttrackrepository.findOne(device.getDeviceid());
			//	log.info("CV2NewDataTest :: Track LOD "+track.getDeviceId());
				if(track!=null)
				{   
				//	log.info("CV2NewDataTest "+imei+" GPRMC  :: Track Not NULL");
				//	log.info("CV2NewDataTest "+imei+" GPRMC gjo :: "+gjo.toString());
			...
			//	log.info("CV2NewDataTest "+imei+" GPS :: "+analogArray.toString());
				...
				{
					log.info("CV2NewDataTest "+imei+" :: NULL");
					JSONArray analogArray =new JSONArray();
					 for (int i = 0; i <analog.length(); i++) {
						 JSONObject analogJsonObject = new JSONObject();
				    	  JSONObject obj = (JSONObject) analog.get(i);
				    	  log.info("CV2NewDataTest "+imei+" GPRMC  :: parameterId :: "+obj.get("Analoginput").toString());
				...
				log.info("CV2NewDataTest "+imei+" Analog :: Analog String");
				Devicemaster device = devicemasterRepository.findByImei(imei);
			...
			//	 List<String> analog1NameList = new ArrayList<>(Arrays.asList(analog1));
			    ...
			    if (msgary[3].contains("1")) {   
				      ...
				      //log.info("CV2NewDataTest Analog  :: "+Arrays.toString(msgStringArray));
				  	...
				  	for(int i=0;i<analog1Param.length;i++)
				  	{
				  		...
				  		AnalogJsonArray.put(analogObj);
							...
							History hst=histroyrepository.saveAndFlush(hist);
							Lasttrack t=lasttrackrepository.saveAndFlush(lTrack);
							log.info("CV2NewDataTest   :: "+t.getDeviceId());
			    }
		else if (msgary[3].contains("5")) {
			
		}else {
			    	log.info("CV2NewDataTest "+imei+" Analog :: Invalid Analog String.");
			    }
			    	
			    }else
				{
			    	log.info("CV2NewDataTest "+imei+" Analog ::NOT NULL");   
			   }
