...
public class CV2EnergyMeterDeviceError {
  
  
	private static final Logger log = Logger.getLogger(CV2EnergyMeterDeviceError.class);
	static SimpleDateFormat sdf = new SimpleDateFormat("ddMMyyHHmmss");
	public static class HandlerCV2EnergyMeterDeviceError extends SimpleChannelUpstreamHandler  {  
		...
		public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
			...
			String msg= (String)e.getMessage();
		 	...
		 	log.info("CV2EnergyMeterDevice:: Msg :Len: "+msg);
			String[] msgary = msg.split(",");
			DecimalFormat twoDForm = new DecimalFormat("#.##");
			String imei=msgary[0].substring(5);
			//log.info("CV2EnergyMeterDevice "+imei+" IMEI :: "+imei);
			String datestr=msgary[10]+msgary[2];
			//log.info("CV2EnergyMeterDevice "+imei+" DATE :: "+datestr);
			
			
			if (msgary[1].contains("$GPRMC")) {
				log.info("CV2EnergyMeterDevice "+imei+" :: "+msg);
			//	log.info("CV2EnergyMeterDevice "+imei+" GPS :: $GPRMC String");
				...
				System.out.println(imei);
				 ...
				 JSONObject parameters = new JSONObject(profile.getParameters());
			      ...
			      {  
			    	  log.info("CV2EnergyMeterDevice "+imei+" IN FORMULA  :: "+obj.get("Analogformula").toString());
			      ...
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Main INPUTS :: "+digitalData);
			      log.info("CV2EnergyMeterDevice "+imei+" GPS GREEN ACMAINS_FAIL :: "+digitalData.substring(1,2));
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Yellow Ornge Fire ::"+digitalData.substring(2,3));
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Yellow Door Door :: "+digitalData.substring(3,4));
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Yellow Black DG_Running_Hrs :: "+digitalData.substring(4,5));
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Yellow Brown DG_Fault :: "+digitalData.substring(5,6));
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Green Black Battry_Low :: "+digitalData.substring(6,7));
			      log.info("CV2EnergyMeterDevice "+imei+" GPS Yellow Red :: PP_Input_Fail "+digitalData.substring(7,8));
				...
				//	String battry=msgary[15];
					...
					double battry=Double.parseDouble(msgary[15].toString()) * analogFormula;
					 log.info("CV2EnergyMeterDevice "+imei+" IN FORMULA>>>>>> :: "+msgary[16]+" :: "+fuel);
					 log.info("CV2EnergyMeterDevice "+imei+" IN FORMULA>>>>>> :: "+battry);
					//DeviceProfile profile=device.getDp();
				Lasttrack track = lasttrackrepository.findOne(device.getDeviceid());
			//	log.info("CV2EnergyMeterDevice :: "+imei+" "+track.getDeviceId());
			//	log.info("CV2EnergyMeterDevice length :: "+imei+" "+track);
				if(track!=null)
				{   
					log.info("CV2EnergyMeterDevice "+imei+" GPRMC  :: Track Not NULL");
				...
				log.info("CV2EnergyMeterDevice "+imei+" GPS :: "+analogArray.toString());
			...
			/*	History hist = new History(device.getDeviceid(), device.getUserId(), new Date(), new Date(),
				...
				{
					log.info("CV2EnergyMeterDevice "+imei+" :: NULL");
					JSONArray analogArray =new JSONArray();
					 for (int i = 0; i <analog.length(); i++) {
						 JSONObject analogJsonObject = new JSONObject();
				    	  JSONObject obj = (JSONObject) analog.get(i);
				    	  log.info("CV2EnergyMeterDevice "+imei+" GPRMC  :: parameterId :: "+obj.get("Analoginput").toString());
				...
				log.info("CV2EnergyMeterDevice "+imei+" Analog :: Analog String");
				Devicemaster device = devicemasterRepository.findByImei(imei);
			...
			//	 List<String> analog1NameList = new ArrayList<>(Arrays.asList(analog1));
			    ...
			    if (msgary[3].contains("1")) { 
			    	String kWh_of_CH1="00000000";
			    	String A_of_CH1="00000000";
			    	String kWh_of_CH2="00000000";
			    	String A_of_CH2="00000000";
			    	String kWh_of_CH3="00000000";
			    	String A_of_CH3="00000000";
			    	String kWh_of_CH4="00000000";
			    	String A_of_CH4="00000000";
			    	String Voltage="00000000";
			    
			    	  log.info("kWh_of_CH1 :: "+msgary[4].split(":")[1]+" :: "+msgary[4].split(":")[1].indexOf("X"));
				      if(msgary[4].split(":")[1].indexOf("X")==-1 && msgary[5].split(":")[1].indexOf("X")==-1) 
				       kWh_of_CH1=msgary[4].split(":")[1].concat(msgary[5].split(":")[1]);
				      
				      log.info("A_of_CH1 :: "+msgary[6].split(":")[1]+" "+msgary[7].split(":")[1]);
				      if(msgary[6].split(":")[1].indexOf("X")==-1 && msgary[7].split(":")[1].indexOf("X")==-1) 
				       A_of_CH1=msgary[6].split(":")[1].concat(msgary[7].split(":")[1]);
				      
				      if(msgary[8].split(":")[1].indexOf("X")==-1 && msgary[9].split(":")[1].indexOf("X")==-1) 
				       kWh_of_CH2=msgary[8].split(":")[1].concat(msgary[9].split(":")[1]);
				      
				      if(msgary[10].split(":")[1].indexOf("X")==-1 && msgary[11].split(":")[1].indexOf("X")==-1)
				       A_of_CH2=msgary[10].split(":")[1].concat(msgary[11].split(":")[1]);
				     
				      if(msgary[12].split(":")[1].indexOf("X")==-1 && msgary[13].split(":")[1].indexOf("X")==-1)
				       kWh_of_CH3=msgary[12].split(":")[1].concat(msgary[13].split(":")[1]);
				     
				      if(msgary[14].split(":")[1].indexOf("X")==-1 && msgary[15].split(":")[1].indexOf("X")==-1)
				       A_of_CH3=msgary[14].split(":")[1].concat(msgary[15].split(":")[1]);
				    
				      if(msgary[16].split(":")[1].indexOf("X")==-1 && msgary[17].split(":")[1].indexOf("X")==-1)
				       kWh_of_CH4=msgary[16].split(":")[1].concat(msgary[17].split(":")[1]);
				     
				      if(msgary[18].split(":")[1].indexOf("X")==-1 && msgary[19].split(":")[1].indexOf("X")==-1)
				       A_of_CH4=msgary[18].split(":")[1].concat(msgary[19].split(":")[1]);
				     
				      if(msgary[20].split(":")[1].indexOf("X")==-1 && msgary[21].split(":")[1].indexOf("X")==-1)
				       Voltage=msgary[20].split(":")[1].concat(msgary[21].split(":")[1]);
				  	...
				  	for(int i=0;i<analog1Param.length;i++)
				  	{
				    	  ...
				    	  //  log.info("CV2EnergyMeterDevice "+imei+" Analog  :: Track Not NULL");
							JSONObject jo = new JSONObject();
							JSONObject analogJsonObject = new JSONObject((mapper.writeValueAsString(track.getAnalogdigidata())));
							JSONArray DigitalJsonArray_old = new JSONArray(analogJsonObject.get("Digital").toString());
							//log.info("CV2EnergyMeterDevice Digital :: "+DigitalJsonArray_old.toString());
							...
							History hst=histroyrepository.saveAndFlush(hist);
							Lasttrack t=lasttrackrepository.saveAndFlush(lTrack);
							log.info("CV2EnergyMeterDevice   :: "+t.getDeviceId());
			    }
		else if (msgary[3].contains("5")) {
			
		}else {
			    	log.info("CV2EnergyMeterDevice "+imei+" Analog :: Invalid Analog String.");
			    }
			    	
			    }else
				{
			    	log.info("CV2EnergyMeterDevice "+imei+" Analog ::NOT NULL");   
			   }
